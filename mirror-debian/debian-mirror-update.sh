...
start_message="🚀 *Mirror sync Debian Archive Updates* script started on $syncDate.%0A Syncing process has begun."
...
if [[ -f "/mnt/mirror/debian/lastSuccess-updates" ]]; then
	rm -v "/mnt/mirror/debian/lastSuccess-updates";
fi
...
do
	...
	echo -n ${releases[*]} | sed 's/ /\n/g' | rsync --no-motd --delete-during --archive --recursive --human-readable --bwlimit=45m --log-file="/home/log/debianupdatesnew-dists--$syncDate".log --files-from=- $masterSource::"$name/dists/" "$localPackageStore/dists/";
...
#						fi
	...
	do
		SECONDS=0;
		rsync --copy-links --files-from="/tmp/$filename" --no-motd --delete-during --archive --recursive --human-readable --log-file="/home/log/debianupdates-pool-$syncDate".log $masterSource::$name "$localPackageStore/" 2>&1;
		exitCode=$?;
	...
	if [[ $exitCode -gt 0 ]]; then
		...
		send_telegram_message "❌ *Rsync Debian Archive Updates failed* all 3 attempts for Debian Updates $masterSource!" #New Feature
		exit 2;
	fi

	echo "$(date +%T) Sync from $masterSource complete, runtime: $SECONDS s";
	# Kirimkan pesan log sukses ke Telegram
	success_message="✅ *Rsync Debian Archive Updates for $masterSource completed successfully* on $syncDate. Runtime: $SECONDS seconds. "
...
#touch "$baseDirectory/lastSuccess";
	touch "$baseDirectory/$name/lastSuccess-updates";

	echo "$(date +%T) Inserting log data into lastSuccess-updates"

	cat "$log_file_success1" "$log_file_success2" >> "$baseDirectory/$name/lastSuccess-updates"
# Notifikasi selesai skrip
	end_message="✅  *Mirror sync Debian Archive Update* script finished on $syncDate. All processes completed."
