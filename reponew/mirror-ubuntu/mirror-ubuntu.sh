...
sourceFolder='/home/reponew/mirror-rsync/mirror-rsync.d';
...
log_file_success1="/home/log/sync-pool-$syncDate.log"  # Log file yang ingin dikirim
log_file_success2="/home/log/sync-dists-$syncDate.log"  # Log file yang ingin dikirim
...
if [[ -f "/mnt/mirror/ubuntu/lastSuccessnew-updates" ]]; then
	rm -v "/mnt/mirror/ubuntu/lastSuccessnew-updates";
fi
...
do
	...
	echo -n ${releases[*]} | sed 's/ /\n/g' | rsync --no-motd --delete-during --archive --recursive --human-readable --bwlimit=45m --log-file="/home/log/sync-dists-$syncDate".log --files-from=- $masterSource::"$name/dists/" "$localPackageStore/dists/";
#	echo -n ${may[*]} | sed 's/ /\n/g' | rsync --no-motd --delete-during --archive --recursive --human-readable --bwlimit=45m --files-from=- $masterSource::"$name/" "$localPackageStore/";
	...
	do
		SECONDS=0;
		rsync --copy-links --files-from="/tmp/$filename" --no-motd --delete-during --archive --recursive --human-readable --log-file="/home/log/sync-pool-$syncDate".log $masterSource::$name "$localPackageStore/" 2>&1;
		exitCode=$?;
	...
	if [[ $exitCode -gt 0 ]]; then
		...
		send_telegram_message "❌ *Rsync failed* all 3 attempts for $masterSource!" #New Feature
		exit 2;
	fi

	echo "$(date +%T) Sync from $masterSource complete, runtime: $SECONDS s";
	# Kirimkan pesan log sukses ke Telegram
	success_message="✅ *Rsync Ubuntu for $masterSource completed successfully* on $syncDate. Runtime: $SECONDS seconds. "
...
# Notifikasi selesai skrip
	end_message="✅ *Mirror sync Ubuntu* script finished on $syncDate.%0A All processes completed."
